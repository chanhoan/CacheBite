...
- "지금 새로고침"은 아키텍처의 디바운스 정책을 따르고, **보이는 provider 전부**를 대상으로 한다. 그중 하나라도 디바운스 중이면 비활성화한다 — "안 바쁜 쪽만 보내기"는 버튼 하나가 상황에 따라 다른 개수의 요청을 내보내게 만들어 예측 가능성을 잃고, 네이티브 디바운스가 어차피 중복을 흘려버리므로 얻는 것도 없다. 라벨은 컬럼 2개면 `Refresh both`, 1개면 `Refresh now`.
