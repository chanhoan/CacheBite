...
**배지는 숫자가 있던 자리, 즉 퍽 중앙에 놓는다.** 같은 사실을 형태만 바꿔 말하는 것이라
provider가 `active`를 드나든다고 문장의 위치가 옮겨 다녀서는 안 된다. 모서리에 붙이면 퍽이
그리는 원 바깥으로 나가 "작은 원의 상태"가 아니라 "작은 원에 붙은 무언가"로 읽힌다.
지름은 글자 크기와 마찬가지로 `SATELLITE_BADGE_RATIO`로 `model.size`에서 픽셀 계산해
`--badge-size`로 넘긴다 — `SystemBadge`의 기본 `2rem`은 오버레이 폭을 모르고, 퍽이 큰 원의
36%인 이 크기에서는 원판을 거의 덮는다. 비율은 세 자리 숫자가 차지하는 폭에 맞춰 두었다.
...
```text
┌────────────────────────────────────────┐
│ Usage                             (✕)  │  ✕ = 패널 닫기 (절대 위치, 흐름 밖)
├──────────────────┬─────────────────────┤
│ Claude ★    Pro  │ Codex        Plus   │  ★ = 주 provider, plan_type은 있을 때만
│ 5시간       82%  │ 5시간         41%   │
│ ▓▓▓▓▓▓▓▓░░       │ ▓▓▓▓░░░░░░          │
│ 리셋까지 1시간 12분│ 리셋까지 3시간 40분  │
│ 주간        94%  │ 주간          58%   │
│ ▓▓▓▓▓▓▓▓▓░       │ ▓▓▓▓▓▓░░░░          │
│ ● Fresh · 2분 전 │ ● Fresh · 5분 전    │
│ 상태 줄 (컬럼마다)│ 상태 줄 (컬럼마다)   │
├──────────────────┴─────────────────────┤
│ 푸터                                    │
...
- 컬럼 수는 **연결된 provider 수에서 파생**된다. 사용자가 고르는 설정이 아니다. `auth_required` / `unavailable`만 미연결로 보며, `error` / `offline` / `loading`은 연결된 provider의 상태이므로 컬럼을 유지한다 — 그러지 않으면 네트워크가 잠깐 끊길 때마다 패널이 1↔2컬럼으로 튄다. 한쪽의 실패가 다른 쪽 표시를 막지 않는다는 provider 독립 원칙은 그대로다.
- **하나도 연결되지 않았으면 둘 다 표시한다.** 숨김은 "다른 쪽이 연결돼 있을 때"만 성립하는 판정이며, 전부 숨기면 두 provider의 로그인 안내가 화면 어디에도 남지 않는다.
- **미연결 provider를 숨긴다는 것은 그 provider를 연결할 수 있다는 사실이 패널에서 사라진다는 뜻이다.** 최초 온보딩(둘 다 미연결)에서는 둘 다 보이므로 경로가 살아 있지만, 한쪽만 쓰는 사용자는 나머지 하나의 존재를 패널로 알 수 없다. "연결된 것만 보여 준다"를 택한 대가이며 의도된 계약이다.
- 컬럼 순서는 `claude`, `codex`로 **고정**이다. primary를 앞으로 끌어오지 않는다 — "주 provider로 설정"을 누른 순간 두 컬럼이 자리를 맞바꿔 방금 읽던 수치가 반대편으로 튄다. ★가 이미 primary를 표시한다.
- 폭은 **380px 고정**이고 높이만 콘텐츠를 따른다. 컬럼이 1개로 접혀도 창 폭은 그대로이며 남은 컬럼이 폭 전체를 차지한다.
- 컬럼 본문은 해당 provider의 `PetUiState` 파생 규칙을 그대로 재사용한다 (주 provider 여부와 무관하게 동일한 도출 함수). freshness 줄과 상태 안내문도 **컬럼마다** 있다.
...
- "지금 새로고침"은 아키텍처의 디바운스 정책을 따르고, **보이는 provider 전부**를 대상으로 한다. 그중 하나라도 디바운스 중이면 비활성화한다 — "안 바쁜 쪽만 보내기"는 버튼 하나가 상황에 따라 다른 개수의 요청을 내보내게 만들어 예측 가능성을 잃고, 네이티브 디바운스가 어차피 중복을 흘려버리므로 얻는 것도 없다. 라벨은 컬럼 수와 무관하게 항상 `Refresh now`다 — 컨트롤의 의미가 어느 쪽이든 같고, provider가 연결·해제될 때마다 라벨이 바뀌면 푸터가 들썩인다.
- **푸터 버튼은 `0.8125rem`(주 동작), `0.75rem`(설정·종료)이다.** 문서 기본값 16px를 상속하면 380px 패널 절반에 `Set Claude as primary`가 들어가지 않아 두 줄로 접힌다. 패널의 나머지 텍스트도 11~13px이므로 16px 쪽이 이질적이었다. `white-space: nowrap`으로 줄바꿈 자체를 막아, 라벨이 길어지면 접히는 대신 넘쳐서 눈에 띄게 한다.
- "주 provider로 설정"은 **보이는 컬럼 중 primary가 아닌 하나**를 겨냥하며 그 이름을 라벨에 싣는다(`Set Codex as primary`). 대상이 없으면(보이는 것이 primary 하나뿐) 비활성이다. primary는 자동 강등되지 않으므로 숨겨진 provider를 가리킬 수 있고, 그때는 남은 컬럼이 곧 대상이 된다.
...
- 탭 스트립이 사라지면서 `✕`가 컨트롤 위에 얹히던 겹침 계약도 함께 폐기되었다. 이제 `✕`가 덮는 것은 헤더의 자체 여백뿐이고, 헤더가 셸 안쪽 폭 전체를 차지한다는 사실만 e2e로 고정한다(`renderer.spec.ts`).
