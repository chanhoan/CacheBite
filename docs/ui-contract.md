...
- 패널은 **지원되는 플랫폼에서 항상 위**로 떠 있고 외부 클릭으로 닫히지 않는다. always-on-top을 거부하는 컴포지터에서는 패널이 뒤로 밀릴 수 있으며, 그 경우 펫 더블클릭의 전면화가 복구 경로다. 닫기 수단은 헤더 우측 상단의 `✕`이며, 이는 패널만 숨긴다(프로세스는 계속 실행된다).
...
- 펫 더블클릭은 **토글이 아니라 표시·전면화**다. 이미 열려 있지만 다른 창 뒤에 있는 패널은 최상단으로 올라오고 포커스를 받는다. 무반응이어서는 안 된다.
