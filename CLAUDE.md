...
- **The orbit walker is bound to the primary, and it sets the overlay's size ceiling.** The mark walking the outer edge always carries `primary_provider` and renders in **both** ring modes — only the shape of its loop changes (§4.5 of the UI contract). Because it reaches further from centre than anything else, `OVERLAY_BOUNDS_FACTOR` clamps `overlaySize`, and it applies in both modes: a mode switch that resized the pet would stop being a display preference. Its `offset-path` is absolute pixels against the overlay box and CSS `path()` takes no percentages, so every constraint on the rendered width — including the bubble's 152 px ceiling — belongs in `overlaySize`, never in a stylesheet the model cannot see.
