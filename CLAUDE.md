...
- Native commands registered in `src-tauri/src/lib.rs` `invoke_handler!`: `get_collector_mode`, `get_provider_states`, `get_settings`, `get_history`, `get_pet_package`, `list_pet_packages`, `get_platform_capabilities`, `save_position`, `refresh_provider`, `update_settings`, `toggle_panel`, `resize_panel`, `hide_panel`, `quit`, `get_update_state`, `check_for_update`, `install_update`. Implementations live in `src-tauri/src/refresh/ipc.rs` **and** `src-tauri/src/update/ipc.rs` — the update commands are in the latter because `refresh/ipc.rs` is already near the 800-line ceiling. `apply_invoke_handler` is duplicated under `#[cfg(feature = "webdriver")]` / `#[cfg(not(...))]` — the webdriver build additionally exposes `get_window_states` for E2E assertions. Adding a command means editing **both** arms plus `NativeCommand`/`command_allowed` in `window/mod.rs`.
