...
python3 scripts/build_updater_manifest.py --self-test   # updater channel-manifest generator assertions
```
...
- Native commands registered in `src-tauri/src/lib.rs` `invoke_handler!`: `get_collector_mode`, `get_provider_states`, `get_settings`, `get_history`, `get_pet_package`, `list_pet_packages`, `get_platform_capabilities`, `save_position`, `refresh_provider`, `update_settings`, `toggle_panel`, `show_pet_menu`, `resize_panel`, `hide_panel`, `quit`, `get_update_state`, `check_for_update`, `install_update`. Implementations live in `src-tauri/src/refresh/ipc.rs` **and** `src-tauri/src/update/ipc.rs` — the update commands are in the latter because `refresh/ipc.rs` is already near the 800-line ceiling. `apply_invoke_handler` is duplicated under `#[cfg(feature = "webdriver")]` / `#[cfg(not(...))]` — the webdriver build additionally exposes `get_window_states` for E2E assertions. Adding a command means editing **both** arms plus `NativeCommand`/`command_allowed` in `window/mod.rs`.
- `src/lib/api/fixtureGateway.ts` is the deterministic renderer-side fixture used in tests/E2E — mirror the real gateway shape when adding methods. It reads query parameters for the variants a spec needs (`?toast=layout`, `?ring=double`); defaults keep every existing spec on the layout it was written against. The `ring=double` specs are the only place a real engine evaluates `offset-path`, the satellite's placement, and the z-order stacking — jsdom computes none of those.

**Native layers (`src-tauri/src/`):**

- `collectors/` — provider collection. `claude.rs` (fixed HTTPS Claude Code OAuth usage endpoint), `codex.rs` (`codex -s read-only -a never app-server` + JSON-RPC `initialize` handshake then `account/rateLimits/read`), `broker.rs` (read-only credential selection), `fallback.rs`, `wsl.rs`. Parsers are isolated, size/time-bounded, and fail with typed provider-scoped outcomes rather than propagating raw errors.
...
- `update/` — in-app updates. `channel.rs` (pure: `channel_for_version`, `manifest_url`, `should_check` and the three cadence constants), `state.rs` (pure: `UpdateStatus`/`UpdateFailure`/`UpdateStateDto`, note truncation), `feed.rs` (`ReleaseFeed` trait + `TauriUpdaterFeed` + `FixtureFeed`, mirroring the `Collector` split), `service.rs` (the actor and its `watch` channel), `ipc.rs` (commands + `emit_update_state`). Cadence: startup +30 s, background sweep every 1 h, on panel reveal with a 15-minute floor, and the manual Settings button with no floor — all four share one `should_check`, differing only by interval.
...
- **Renderer command authority is per-window.** `window::command_allowed(window_label, command)` is the allowlist: `overlay` may only read state, save its position, `toggle_panel`, and `show_pet_menu` (the popup request — item actions run in Rust via `pet_menu_action`); `panel` owns `get_history`, `list_pet_packages`, `refresh_provider`, `update_settings`, `resize_panel`, `hide_panel`, and `quit`. Pet-menu **ids and dispatch** live in `window/mod.rs` (policy); the menu's **UI copy** lives in `refresh/ipc.rs` (`pet_menu_panel_label` and the item labels) so wording changes never touch policy tests. Menu build/popup failures surface as `IpcError::MenuUnavailable`. The menu's "Hide pet" is an explicit hide (`hide_overlay_from_menu`), not a toggle — the hotkey stays live while the popup is open, so a toggle could surprise-reshow a pet the user just hid. Any other label is denied outright. A new command must be added to `NativeCommand`, to this allowlist, to both `apply_invoke_handler` arms, and to `AppGateway` — skipping the allowlist silently denies it at runtime.
...
- **Ring mode is display-only.** `ring_mode` (`single`/`double`, schema v6) decides whether the overlay draws a satellite ring beside the big one. The big ring is **always** `primary_provider` — there is no swap or auto-demotion on a disconnected provider — and the satellite is always `secondaryProvider(primary)`, derived and never written back. It must not reach collection, refresh scheduling, notification routing, or the pet mood: the moment a display preference changes behavior, the two stop being separable. An unconnected satellite keeps its ring and shows a system badge, so the setting always describes the screen.
- **Each ring carries its own orbiting mark, and the satellite's sets the overlay's size ceiling.** The big ring's mark always carries `primary_provider` and renders in **both** ring modes, on the same circle either way; the satellite's mark appears only in `double` and always carries `secondaryProvider(primary)` — the same derived value the satellite ring uses, never stored. The two circles are separate objects now, so there is no union outline: each mark walks its own full circle, and they share one direction roll because opposite rotations stop reading as a single scene (§4.5 of the UI contract). The satellite's mark reaches further from centre than anything else, so `OVERLAY_BOUNDS_FACTOR` (1.83) clamps `overlaySize` from its right-hand extreme, and it applies in both modes: a mode switch that resized the pet would stop being a display preference. The reach it measures includes each mark's **rotated** bounding box — `offset-rotate: auto` turns the mark with its path, so treating it as an axis-aligned square understates the reach and a package asking for exactly the clamped size clips on the right. Headroom is ~1.9 points — every bundled pet declares `defaultSize` 128 and the clamp allows 131 — so growing `SATELLITE_DISTANCE` or `SATELLITE_SIZE`, or flattening `SATELLITE_BEARING` toward horizontal, all shrink the pet for real; a square window makes a diagonal offset cheaper than a sideways one. `SATELLITE_BEARING` equals `TANGENT_HALF_ANGLE` by construction: that is the steepest bearing keeping the satellite's lowest point level with the big ring's, and at the equality both circles rest on one horizontal line. `offset-path` is absolute pixels against the overlay box and CSS `path()` takes no percentages, so every constraint on the rendered width — including the bubble's 152 px ceiling, which the current geometry keeps below the binding threshold — belongs in `overlaySize`, never in a stylesheet the model cannot see.
...
- **Update authority and channel.** The renderer never talks to `tauri-plugin-updater`; the capability files must contain no `updater:*` permission, or the webview gains a bypass around `command_allowed`. The channel is derived from the running version (`update::channel_for_version`) and never persisted — a stored channel could record a beta opt-in no migration would undo. `stable.json` never lists a pre-release, so a release build cannot be pulled onto beta. `bundle.createUpdaterArtifacts` stays `false` in the committed config; CI enables it through the generated `src-tauri/tauri.release.conf.json` so a contributor without the signing key can still build. That same generated config makes the **git tag** the source of truth for `version` — the committed `0.1.0` ranks _above_ every `0.1.0-beta.N`, so shipping it would make the updater offer nothing. The MSI additionally needs a numeric `bundle.windows.wix.version` because WiX rejects a non-numeric pre-release. A manifest missing this platform's key returns `ArtifactMissing` on **every** check, including when the client already matches the manifest version — `Updater::check` resolves the artifact before it consumes the version comparison — so `ArtifactMissing` is never evidence that an update is waiting.
